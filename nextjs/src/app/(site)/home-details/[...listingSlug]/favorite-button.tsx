...
export default function FavoriteButton({ initial_favorite, listing_id }: FavoriteButtonProps) {
  ...
  return (
    <>
      ...
      <button
        type='button'
        className={`ml-4 flex items-center justify-center rounded-md p-3 ${
          favorited ? 'text-rose-400 hover:text-rose-500' : 'text-gray-400 hover:text-gray-500'
        } hover:bg-gray-100 `}
        onClick={handleFavorite}
      >
        {favorited ? (
          <HeartIconSolid className='h-6 w-6 shrink-0' aria-hidden='true' />
        ) : (
          <HeartIconOutline className='h-6 w-6 shrink-0' aria-hidden='true' />
        )}
